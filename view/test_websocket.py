...
class WebSocketThread(threading.Thread):
    ...
    def run(self):
        ...
        asyncio.get_event_loop().run_until_complete(
            websockets.serve(self.listen, "localhost", 6789)
        )
        # keep thread running
    ...
    async def notify_users(self):
        ...
        if self.USERS:
            ...
            await asyncio.wait(tasks)  # Pass tasks to asyncio.wait

    async def action(self):
        """this is an action which will be executed when user presses a button"""
        if self.USERS:
