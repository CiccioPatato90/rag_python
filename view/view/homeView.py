...
from model.event_classes import Observer, ConcreteSubject, EventType

class ChatInterface(Frame, Observer):
    ...
    def send_message_insert(self, message):
        ...
        #t1.start()
        #time.sleep(1)
        pub.sendMessage("SendToLLM_Button_Pressed", query=user_input)
        
    def update(self, subject: ConcreteSubject, event_type: EventType) -> None:
        if event_type == EventType.NEW_RESPONSE:
            state = subject.get_state()
            self.update_response(state.last_response)
